        ...
        <div className="flex items-center gap-10 px-10 py-[18px] border-t border-gray-200 bg-[rgba(245,245,245,0.7)]">
            ...
            <div className="flex gap-8 flex-1">
                {["CHAINALYSIS", "Elliptic", "TRM Labs", "Nansen", "Dune"].map((name) => (
                    <span
                        key={name}
                        className="text-[0.8rem] font-semibold tracking-wide text-gray-400 hover:text-gray-500 transition-colors cursor-default"
                    >
                        {name}
                    </span>
                ))}
            </div>
            <div className="flex items-center gap-3 text-[0.75rem] text-gray-400 whitespace-nowrap">
                <RunStatusPill />
            </div>
        </div>
    );
