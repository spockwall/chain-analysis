...
- A simulated peel-chain laundering pattern (suspect EOA → mixer → multiple wallets)
- A fan-out structuring pattern (whale → 5 EOAs → CEX)
- Transaction nodes with SENT/RECEIVED relationships
...
NODES = [
    ...
    # ── Tornado Cash instances (critical) ─────────────────────────────────────
    {
    ...
    # ── Simulated peel chain (suspect → mixer → exit wallets) ─────────────────
    # The suspect originates funds and routes through Tornado Cash
    {
        "address": "0xaaaa000000000000000000000000000000000001",
        "labels": ["Entity", "EOA"],
        "props": {"name": "Suspect Origin Wallet", "risk_level": "high", "entity_type": "EOA",
                  "tx_count_out": 12, "volume_out": "580000000000000000000"},
    },
    {
        "address": "0xaaaa000000000000000000000000000000000002",
        "labels": ["Entity", "EOA"],
        "props": {"name": "Peel Hop 1", "risk_level": "high", "entity_type": "EOA",
                  "tx_count_in": 1, "tx_count_out": 1, "volume_in": "100000000000000000000",
    ...
    },
    {
        ...
        "props": {"name": "Peel Hop 2", "risk_level": "high", "entity_type": "EOA",
                  "tx_count_in": 1, "tx_count_out": 1, "volume_in": "99000000000000000000",
    ...
    },
    {
        ...
        "props": {"name": "Peel Hop 3 (exit)", "risk_level": "medium", "entity_type": "EOA",
                  "tx_count_in": 1, "tx_count_out": 1},
    },

    # ── Simulated fan-out structuring (whale → 5 smurfs → CEX) ───────────────
    {
        "address": "0xbbbb000000000000000000000000000000000001",
        "labels": ["Entity", "EOA"],
        "props": {"name": "Whale (structuring source)", "risk_level": "high", "entity_type": "EOA",
                  "tx_count_out": 5, "volume_out": "490000000000000000000"},
    },
    {
        "address": "0xbbbb000000000000000000000000000000000002",
        "labels": ["Entity", "EOA"],
        "props": {"name": "Smurf 1", "risk_level": "medium", "entity_type": "EOA",
                  "tx_count_in": 1, "tx_count_out": 1, "volume_in": "98000000000000000000"},
    },
    {
...
TRANSACTIONS = [
    # ── Peel chain ────────────────────────────────────────────────────────────
    # Suspect → Tornado Cash 10 ETH (multiple deposits)
    {
        "hash": "0xaaa001",
    ...
    # Suspect also interacts with Uniswap (layering)
    {
        "hash": "0xaaa007",
        "from_address": "0xaaaa000000000000000000000000000000000001",
    ...
    # Whale also uses Aave (to obscure source)
    {
        "hash": "0xbbb020",
        "from_address": "0xbbbb000000000000000000000000000000000001",
    ...
    # Cross-pattern link: peel chain exit also deposits to Tornado Cash 100 ETH
    {
        "hash": "0xaaa010",
        "from_address": "0xaaaa000000000000000000000000000000000004",
...
async def seed(uri: str, user: str, password: str) -> None:
    ...
    print(f"Seeding {len(NODES)} nodes and {len(TRANSACTIONS)} transactions...\n")
    ...
    async with driver.session() as session:
        ...
        for node in NODES:
            ...
            if has_apoc:
                ...
                # Without APOC we can't dynamically add labels in one query,
                # so we use individual label-specific MERGE statements.
    ...
    print(f"\n── Done ──────────────────────────────────────────")
    print(f"  Entity nodes in graph      : {node_count}")
    print(f"  Transaction nodes in graph : {tx_count}")
    print(f"\nOpen Neo4j Browser at http://localhost:7474 to explore.")
    print(f"Or use the app at http://localhost:3000 and search for:")
    print(f"  0xaaaa000000000000000000000000000000000001  (suspect — peel chain)")
    print(f"  0xbbbb000000000000000000000000000000000001  (whale — structuring)")
    print(f"  0x910cbd523d972eb0a6f4cae4618ad62622b39dbf  (Tornado Cash 10 ETH)")
